...
public class BaseCamPacketSlim
{
    #region defines
    ...
    public BaseCamPacketSlim()
    {
        data = new byte[SBGC_CMD_MAX_BYTES];
        data[INDEX_SOF] = SBGC_STC_V2;
    }

    public BaseCamPacketSlim(byte _id) : this(_id, SBGC_CMD_MAX_BYTES)
    {
            
    }

    public BaseCamPacketSlim(byte _id, int SIZE_PACKET)
    {
    ...
    public BaseCamPacketSlim clone()
    {
        var cloned = new BaseCamPacketSlim(ID);
        cloned.pos = pos;
    ...
    internal BaseCamPacketSlim UpdateChecksums()
    {
        //header 
